...
fun Context.getDrawableCompat(@DrawableRes drawableRes: Int) = ContextCompat
...
fun View.showIf(predicate: Boolean) {
    this.takeIf { predicate }?.show() ?: hide()
}
...
fun Double.roundToADecimalPlace() =
        BigDecimal(this)
...
inline fun <reified T : Activity> Context.startActivityClearTop(b: Bundle? = null,
...
inline fun <reified T : Activity> Activity.startActivityForResult(b: Bundle? = null,
                                                                  requestCode: Int,
                                                                  flags: Int = 0) {
    val i = Intent(this, T::class.java).addFlags(flags)
    if (b != null) i.putExtras(b)
...
inline fun <reified T : Activity> Fragment.startActivityForResult(b: Bundle? = null,
                                                                  requestCode: Int,
                                                                  flags: Int = 0) {
    val i = Intent(activity, T::class.java).addFlags(flags)
    if (b != null) i.putExtras(b)
